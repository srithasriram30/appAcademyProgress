...
function bothStringsIncluded(sentence, word1, word2){
    let first = sentence.toLowerCase().indexOf(word1.toLowerCase()) != -1;
    let second = sentence.toLowerCase().indexOf(word2.toLowerCase()) != -1;
...
function countConsonants(word){
    ...
    for (let i = 0; i < word.length; i++) {
        ...
        if(vowels.indexOf(word[i]) == -1 && word[i] != " "){
            count++;
        }
    }
    return count;
